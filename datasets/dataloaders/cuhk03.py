...
@dataclass
class SiameseCuhk03Dataset(Dataset):
    data_dir: Path
    transform: transforms.Compose = None

    @staticmethod
    def get_id(path: Path) -> str:
        splits = str(path.parts[-1]).split("_")
        return splits[0] + "_" + splits[1]

    def __post_init__(self):
        self.train_dir = self.data_dir / "images_labeled"
        self.image_list = list(self.train_dir.glob("*.png"))
